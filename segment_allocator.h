...
public:
    ...
    SegmentAllocator() {
        ...
        for (uint32_t index = 0; index < total_blocks_; ++index) {
            OneBlock& block = all_blocks_[index];
            block.previous = index - 1;
            block.next = index + 1;
    ...
    uint32_t Allocate(uint16_t size) {
        ...
        if (size_info.head_block == null_block_) {
            ...
            if (block_index == null_block_) {
                ...
                all_blocks_[block_index].InitializeForSize(size);
	...
	bool Free(uint32_t start, uint16_t size) {
...
private:
    struct OneBlock {
        ...
        void InitializeForSize(uint16_t segment_size) {
            one_segment_size = segment_size;
            total_segments = block_size / segment_size;
            free_segments = total_segments;
            // std::cerr << "\tInitialized block, segment=" << one_segment_size << ", number=" << total_segments << "\n";
        }
        ...
        uint16_t Allocate() {
            ...
            uint16_t group = __builtin_ctz(~group_mask);
            if ((group >= sizeof(masks) / sizeof(masks[0])) || ((~masks[group]) == 0)) {
                return error_in_block_;
            }
            uint16_t index_in_group = __builtin_ctz(~masks[group]);
